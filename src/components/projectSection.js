...
const ProjectSection = () => {
  return (
    <div className="project-section px-2 xs:px-4 sm:px-6 lg:px-8 py-4 sm:py-8" id="projects">
      ...
      <div className="project-container grid grid-cols-1 rounded-3xl gap-6 lg:gap-8">
        {projects.map((project, index) => (
          <div
            key={project.id}
            className={`project-card relative bg-cover bg-center p-4 sm:p-6 lg:p-8 shadow-lg ${
              index % 2 === 1 ? "sm:project-card-left" : ""
            }`}
          ...
          >
            ...
            {/* Project Content */}
            <div
              className={`project-content mt-4 sm:mt-6 lg:mt-8 ${
                index % 2 === 1 ? "text-left sm:text-right" : "text-left"
              }`}
            >
              {/* Skills Section */}
              <div className="project-skills-container flex flex-wrap gap-2 mb-2 sm:mb-4 justify-start sm:justify-end">
                {project.skills.map((skill, index) => (
                  <img
                    ...
                    src={skill.src}
                    alt={skill.alt}
                    className="project-skill w-6 h-6 xs:w-8 xs:h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 object-contain"
                  />
                ))}
              </div>
              ...
              {/* Buttons and Links */}
              <div className="btn-grp flex flex-wrap gap-2 sm:gap-4 items-center">
