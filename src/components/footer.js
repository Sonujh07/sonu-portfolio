...
const Footer = () => {
  ...
  <div className="absolute z-0 left-4 xs:left-6 sm:left-8 md:left-9 h-7 w-7 xs:h-8 xs:w-8 md:h-9 md:w-9 rounded-full bg-coolBlue"></div>
  
  {/* Text size and alignment based on breakpoints */}
  <p className="text-lg xs:text-xl sm:text-2xl font-raleway font-semibold z-10 text-black">
    Sonu.tech
  </p>
</div>
      ...
      <div className="flex flex-col gap-y-4 md:gap-y-7 w-full md:w-1/5 text-center md:text-left">
        ...
        className="border h-12 w-12 text-center p-1 rounded-lg transition duration-700 ease-in-out transform  hover:bg-white hover:text-blue-400" />
          ...
          <a href="https://www.linkedin.com/in/sonu-kumar-bca/" target="_blank">
          <FaLinkedin
